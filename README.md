...
- **[dandelion](https://github.com/danogurtsov/dandelion)** — reconstructs a protocol's on-chain architecture from addresses, not a repo: resolves code (verified source or decompiled bytecode), reads real state, and autonomously crawls proxies, roles, reserves and cross-chain deployments into one typed architecture graph. From a single Aave Pool address it maps the whole protocol. Model-agnostic LLM reasoning loop; hexagonal, fully typed, 106 tests + live evals (Python).
